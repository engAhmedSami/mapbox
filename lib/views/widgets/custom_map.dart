...
class _CustomMapState extends State<CustomMap> {
  ...
  void _onStyleLoaded(StyleLoadedEventData styleLoadedEventData) async {
    ...
    // Add this flag at the beginning
    ...
    try {
      ...
      // Wait a short delay to ensure the style is fully loaded
      await Future.delayed(const Duration(milliseconds: 100));

      // Update other map elements after being sure initialization is complete
      if (_locationController.currentLocation != null && mounted) {
        _updateDirectionArrow(_locationController.currentLocation!);
      }

      // Clear the _isStyleLoading flag
      _isStyleLoading = false;

      // Now that style is fully loaded and initialized, update the map
      if (mounted) {
        if (_locationController.currentLocation != null) {
          _goToCurrentLocation();
        }
  ...
  Future<void> _updateRouteOnMap(RouteModel route) async {
    if (!mounted ||
        _mapboxMap == null ||
        !_layersInitialized ||
        _isStyleLoading ||
